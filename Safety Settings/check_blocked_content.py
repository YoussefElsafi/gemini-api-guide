...
if response.prompt_feedback and response.prompt_feedback.block_reason: # Check Prompt Feedback
        print("Prompt blocked:", response.prompt_feedback.block_reason)
elif response.candidates and response.candidates[0].finish_reason == types.FinishReason.SAFETY: # Check Safety Ratings
    print("Response blocked. Safety ratings:")
    for safety_rating in response.candidates[0].safety_ratings:
        print(f"  - {safety_rating.category}: {safety_rating.probability}")
else:
